...
const Table = ({ heads, items, onClickUpdate, onClickDelete, type }: ITableProps) => {
    ...
    (key: string, value: any, type?: string) => {
      ...
      switch (key) {
        case 'createdAt':
          ...
          break;
        case 'images':
          data = <div className="max-w-[125px]">
            <ImagesSlider images={value} />
          </div>
          break;
        case 'buyer':
          data = <ShowDataModal data={value} title='Show buyer data' />
          break;
        case 'orderItems':
          data = <ShowOrderedData data={value} title='Show ordered items' />
          break;
        case 'orderState':
          data = <EliteStatusBox status={`${value}`} type='order' />
          break;
        case 'status':
          data = <EliteStatusBox status={`${value}`} type={type} />
          break;
        case 'startPoint':
          data = <>الشركة</>
          break;
  ...
  // const returnedTableItem = (key: string, value: any, type?: string) => {
  //   let data: any = value;
  //   switch (key) {
  //     case 'createdAt':
  //       data = formatDate(value)
  ...
  // }
  // console.log(items)
  const Buttons = ({ updateHandler, deleteHandler }: IActionButtons) => {
    return (
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                  {
                    items.map((item) => (
                      <tr key={item._id}>
                        {
                          heads.map((head, i) => (
                            i !== heads.length - 1 ? <TableItem key={head.key} category={head.key}>
                              {returnedTableItem(head.key, item[head.key], type)}
                              {/* {head.key === 'createdAt' ? formatDate(item[head.key]) : item[head.key]} */}
                            </TableItem> :
                              <Buttons
                                updateHandler={() => onClickUpdate(item._id)}
                                deleteHandler={() => onClickDelete(item._id)}
                                key={head.key}
                              />
                          ))
                        }
